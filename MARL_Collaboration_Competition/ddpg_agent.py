...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, num_agents):
        ...
        print('Cuda is available is: {}'.format(torch.cuda.is_available()))
        print(device)
    ...
    def act(self, state, add_noise=True):
        ...
        # Add noise to multiple agent actions
        if add_noise:
    ...
    def learn(self, experiences, gamma):
        ...
        # Compute Q targets for current states (y_i) using actor's action predictions for next state
        Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))
        
        # Compute critic loss
        # Get values from critic based on current state and action taken
        ...
        # Minimize the loss and backprpagate
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step()
        
        # ----------------------- update target networks ----------------------- #
        if self.learn_step % UPDATE_EVERY == 0:
...
class ReplayBuffer:
    """Shared fixed-size buffer to store experience tuples."""
    ...
    def add(self, state, action, reward, next_state, done, num_agents):
        """Add agents' new experience to memory."""
